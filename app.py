...
BLOCK_SIZE = 2500  # Reduce bloques para evitar timeout

def limpiar_termino(termino):
    termino = termino.strip("-•*·•—–—0123456789. \t\r\n")
    if not termino or len(termino) < 2:
        return ""
    if re.match(r"^(EN|ES|DE|FR|IT|PT)[\s:-]", termino):
        termino = re.sub(r"^(EN|ES|DE|FR|IT|PT)[\s:-]", "", termino).strip()
    if termino.isupper():
        return termino
    elif re.match(r"^[A-Z][a-z]+(\s[A-Z][a-z]+)*$", termino):
        return termino
...
def get_terms_openai(text, source_lang, target_lang):
    if not text.strip():
        return ""
    prompt = (
        f"Del siguiente texto en {source_lang}, extrae aprox. 10 términos clave por cada 1000 palabras, uno por línea. "
        f"Para cada término, proporciona una traducción sugerida al {target_lang} separada por tabulador. "
        f"Evita repeticiones. Conserva nombres propios y siglas, y usa minúsculas para términos generales.\n\nTexto:\n{text}"
    )
    try:
    ...
    for block in blocks:
        ...
        for line in raw.splitlines():
            ...
            if "\t" in line:
                ...
                if term["source"] and term not in all_terms:
